  ...
  <div class="detection-result-card">
    ...
    <div class="card-body">
      ...
      <div class="result-images-grid" v-if="isBatch && batchImages.length > 0">
        <div v-for="(img, index) in batchImages" :key="index" class="grid-image" @click="previewImage(img)">
      ...
      <div class="result-stats">
        ...
        <!-- 类别统计表格 -->
        <el-table v-if="classCountsArray.length > 0" :data="classCountsArray" size="small" style="margin-top: 12px">
...
const annotatedImageSrc = computed(() => {
  ...
  if (props.result.annotated_image_base64) return `data:image/jpeg;base64,${props.result.annotated_image_base64}`
  return null
})
...
const batchImages = computed(() => {
  ...
  return props.result.annotated_images.map((img) => ({
    ...
    src: img.annotated_image_url || `data:image/jpeg;base64,${img.annotated_image_base64}`,
  }))
})
...
.detection-result-card { margin-top: 12px; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden; }
.card-header { display: flex; align-items: center; gap: 8px; padding: 8px 12px; background: #f5f7fa; border-bottom: 1px solid #e0e0e0; font-weight: 600; font-size: 14px; }
.card-body { display: flex; gap: 16px; padding: 12px; }
.result-image { flex: 1; min-width: 0; img { width: 100%; max-height: 300px; object-fit: contain; border-radius: 4px; cursor: pointer; transition: opacity .2s; &:hover { opacity: .8; } } }
