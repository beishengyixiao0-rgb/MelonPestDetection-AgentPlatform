<template>
  <div class="chat-page">
    ...
    <div class="message-list" ref="messageListRef">
      <div v-for="(msg, index) in agentStore.messages" :key="index" :class="['message-item', `message-${msg.role}`]">
        <!-- 用户消息 -->
    ...
    <div class="quick-actions">
      <el-button @click="handleQuickDetect('single')" :disabled="agentStore.isLoading" :icon="Camera">单图检测</el-button>
      <el-button @click="handleQuickDetect('batch')" :disabled="agentStore.isLoading" :icon="FolderOpened">批量/ZIP</el-button>
      <el-button disabled :icon="VideoCamera">视频</el-button>
...
import { Camera, Close, FolderOpened, Monitor, Paperclip, Promotion, VideoCamera } from '@element-plus/icons-vue'
...
async function handleQuickDetect(type) {
  if (type === 'single') {
    ...
    input.onchange = async (e) => {
      ...
      // 添加用户消息（显示文件名）
      agentStore.addMessage({ role: 'user', content: `[快捷检测] ${file.name}`, image: file.name, imagePreview: URL.createObjectURL(file) })

      // 添加加载占位
      agentStore.addMessage({ role: 'assistant', content: '正在检测中...', loading: true })
      agentStore.setLoading(true)
      ...
      try {
        ...
        lastMsg.loading = false
      } finally { agentStore.setLoading(false) }
    }
    input.click()
  } else if (type === 'batch') {
    // 批量检测（支持多选 + ZIP）
    ...
    input.onchange = async (e) => {
      ...
      const formData = new FormData()
      if (isZip && files.length !== 1) { ElMessage.warning('ZIP 文件请单独选择'); return }
      ...
      if (isZip && files.length === 1) {
        ...
        agentStore.addMessage({ role: 'user', content: `[快捷检测] ${files.length} 张图片`, images: imagePreviews })
      }

      agentStore.addMessage({ role: 'assistant', content: '正在批量检测中...', loading: true })
      agentStore.setLoading(true)
      try {
...
onMounted(() => {
  ...
  if (agentStore.messages.length === 0) {
    agentStore.addMessage({ role: 'assistant', content: '你好！我是 RSOD 目标检测智能体助手。\n\n你可以：\n- 上传一张图片，让我帮你检测目标\n- 使用下方的快捷按钮直接触发检测\n- 用自然语言描述你的需求\n\n试试发一张图片给我吧！' })
  }
})
</script>

<style lang="scss" scoped>
.chat-page { display: flex; flex-direction: column; height: 100%; background: #f5f5f5; }
/* ── 消息列表 ── */
.message-list { flex: 1; overflow-y: auto; padding: 20px; }
.message-item { display: flex; margin-bottom: 16px; &.message-user { justify-content: flex-end; } &.message-assistant { justify-content: flex-start; } }
.message-bubble { max-width: 70%; padding: 12px 16px; border-radius: 12px; line-height: 1.5; word-break: break-word; }
...
.markdown-body { /* markdown 渲染后的 HTML 样式 */ h1, h2, h3 { margin-top: 8px; margin-bottom: 4px; } table { border-collapse: collapse; width: 100%; margin: 8px 0; } th, td { border: 1px solid #e0e0e0; padding: 4px 8px; } code { background: #f0f0f0; padding: 2px 4px; border-radius: 3px; } }
.typing-indicator { display: flex; gap: 4px; span { width: 6px; height: 6px; background: #999; border-radius: 50%; animation: typing 1.2s infinite; } span:nth-child(2) { animation-delay: .2s; } span:nth-child(3) { animation-delay: .4s; } }
/* ── 快捷操作栏 ── */
.quick-actions { display: flex; gap: 8px; padding: 12px 20px; border-top: 1px solid #e0e0e0; background: white; }
/* ── 输入区域 ── */
.input-area { display: flex; gap: 8px; padding: 12px 20px; border-top: 1px solid #e0e0e0; background: white; .el-input { flex: 1; } }
/* ── 附件预览 ── */
.message-attachment { margin-top: 8px; img { max-width: 200px; border-radius: 8px; border: 1px solid #e0e0e0; } }
/* ── 多图附件网格 ── */
.message-attachments-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(80px, 1fr)); gap: 8px; margin-top: 8px; img { width: 100%; height: 80px; object-fit: cover; border-radius: 6px; border: 1px solid #e0e0e0; } }
/* ── 工具调用信息 ── */
.tool-call-info { margin-top: 8px; padding: 4px 8px; background: #f5f5f5; border-radius: 4px; font-size: 12px; color: #666; }
@keyframes typing { 0%, 60%, 100% { opacity: .3; transform: translateY(0); } 30% { opacity: 1; transform: translateY(-4px); } }
@media (max-width: 640px) { .message-bubble { max-width: 88%; } .quick-actions { padding: 12px; overflow-x: auto; } .input-area { padding: 12px; } }
</style>
