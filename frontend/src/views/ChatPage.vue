...
const AGENT_ATTACHMENT_ACCEPT = "image/*,.zip,video/*";
const VIDEO_FILE_SUFFIXES = new Set([
  ".mp4",
  ".avi",
  ".mov",
  ".mkv",
...
const hasReadyCameraAttachment = computed(() =>
  uploadQueue.value.some(
    (item) => item.mode === "camera" && item.status === "success",
  ),
);
...
const setUploadMode = (mode) => {
  ...
  if (mode === "agent-image") {
    inputAccept.value = AGENT_ATTACHMENT_ACCEPT;
...
const isVideoFile = (file) =>
  ...
  VIDEO_FILE_SUFFIXES.has(getFileSuffix(file));

const isZipFile = (file) => getFileSuffix(file) === ".zip";
...
const uploadAttachment = async (item) => {
  ...
  try {
    ...
    item.type = result?.file_type || result?.data?.file_type || item.type;
...
const createUploadItem = (file) => {
  ...
  const previewUrl = isZip ? "" : URL.createObjectURL(file);

  // 上传回调会持续修改这个对象，使用 reactive 才能立即刷新进度与成功状态。
  const item = reactive({
    id: `${Date.now()}-${Math.random().toString(16).slice(2)}`,
    name: file.name,
    type: isVideo ? "video" : isZip ? "zip" : "image",
    previewUrl,
    progress: 0,
    status: "uploading",
...
const handleFileSelection = (files) => {
  ...
  const selectedFiles =
    uploadMode.value === "batch" || uploadMode.value === "agent-image"
      ? normalizedFiles
...
const persistQuickDetectionHistory = async ({
  ...
  try {
    ...
    const attachments = [...attachmentUrls, ...uploadedAttachments].filter(
      Boolean,
    );
  ...
  input.onchange = async (event) => {
    ...
    if (type === "single") {
      ...
      try {
        ...
        loadingMessage.content = tr("chat.quick.singleDone", {
          count: result.total_objects ?? 0,
        });
        loadingMessage.loading = false;
        loadingMessage.type = "diagnosis";
        loadingMessage.detectionResult = result;
        const annotatedBase64 =
          result.annotated_image_base64 || result.annotated_image;
        ...
        loadingMessage.content = tr("chat.quick.singleFailed", {
    ...
    try {
      ...
      if (result.error) {
        loadingMessage.content = tr("chat.quick.batchFailed", {
      ...
      loadingMessage.content = tr("chat.quick.batchDone", {
        count: displayResult.total_objects ?? 0,
      });
      loadingMessage.loading = false;
      loadingMessage.type = "diagnosis";
  ...
  try {
    for (let attempt = 0; attempt < VIDEO_POLL_MAX_ATTEMPTS; attempt += 1) {
      ...
      const sourceVideoUrl =
        ...
        payload.result?.source_video_url ||
        payload.detection_result?.source_video_url ||
        payload.data?.source_video_url ||
        payload.data?.result?.source_video_url;

      if (
        sourceVideoUrl &&
        userVideoMessage?.type === "video" &&
        userVideoMessage.videoUrl !== sourceVideoUrl
      ) {
        if (userVideoMessage.videoUrl?.startsWith("blob:")) {
          URL.revokeObjectURL(userVideoMessage.videoUrl);
        }
        userVideoMessage.videoUrl = sourceVideoUrl;
      }
      ...
      if (
        ...
        loadingMessage.content = tr("chat.video.done", {
          count: result.total_objects ?? 0,
        });
        loadingMessage.loading = false;
        loadingMessage.type = "diagnosis";
...
async function handleVideoDetect(selectedFile = null) {
  ...
  input.onchange = async (e) => {
    ...
    try {
      ...
      // 轮询检测进度
      const result = await pollVideoProgress(
        taskId,
        loadingMessage,
        userVideoMessage,
      );
      ...
      loadingMessage.content = tr("chat.video.failed", {
        message: getErrorMessage(error),
      });
...
const parseAgentToolResult = (result) => {
  const parsed = typeof result === "string" ? JSON.parse(result) : result;
  if (!parsed || typeof parsed !== "object") return parsed;

  const nested =
    parsed.data?.result ||
    parsed.data?.detection_result ||
    parsed.result ||
...
const validateAgentAttachments = (attachments) => {
  const agentAttachments = attachments.filter((item) => item.mode === "agent-image");
  if (!agentAttachments.length) return true;

  const hasVideo = agentAttachments.some((item) => item.type === "video");
  const hasZip = agentAttachments.some((item) => item.type === "zip");
  const hasImage = agentAttachments.some((item) => item.type === "image");

  if (hasVideo && (agentAttachments.length > 1 || hasZip || hasImage)) {
    ElMessage.warning("Agent 视频检测请一次只上传一个视频文件");
    return false;
  }

  if (hasZip && (agentAttachments.length > 1 || hasVideo || hasImage)) {
    ElMessage.warning("Agent ZIP 检测请一次只上传一个 ZIP 文件");
    return false;
  }
...
const sendMessage = async () => {
  ...
  const userMessage = {
    role: "user",
    content: content || "请帮我分析这些附件",
  };

  if (attachments.length === 1) {
    if (attachments[0].type === "video") {
      userMessage.type = "video";
      userMessage.videoUrl = attachments[0].previewUrl;
      userMessage.videoFallbackUrl = attachments[0].previewUrl;
    } else if (attachments[0].type === "zip") {
      userMessage.type = "file";
      userMessage.fileName = attachments[0].name;
      userMessage.fileType = "ZIP";
    } else {
    ...
    userMessage.fileNames = attachments.map((item) => item.name);
  }
    ...
    },
    {
      onMessage: (event) => {
        ...
        if (["tool_result", "tool_end"].includes(event.type) && event.result) {
          try {
            const detectionResult = parseAgentToolResult(event.result);
            ...
            const isDetectionResult =
              detectionTools.includes(event.tool) ||
              ...
              "annotated_video_url" in (detectionResult || {}) ||
              "source_video_url" in (detectionResult || {}) ||
              "key_frames" in (detectionResult || {});
            ...
            if (isDetectionResult) {
              ...
              if (
                userMessage.type === "video" &&
                detectionResult?.source_video_url &&
                userMessage.videoUrl !== detectionResult.source_video_url
              ) {
                userMessage.videoFallbackUrl =
                  userMessage.videoFallbackUrl || userMessage.videoUrl;
                userMessage.videoUrl = detectionResult.source_video_url;
              }
            }
          } catch (error) {
...
onMounted(async () => {
  ...
  if (pendingPrompt.mode === "agent-image" && pendingFiles.length) {
    ...
    const pendingItems = pendingFiles
      .map((file) => createUploadItem(file));
    message.value = pendingPrompt.content || "请帮我分析这些附件";
...
const startNewDiagnosis = () => {
  messages.value.forEach((item) => {
    ...
    if (item.videoFallbackUrl?.startsWith("blob:"))
      URL.revokeObjectURL(item.videoFallbackUrl);
...
@media (max-width: 900px) {
  ...
  .sidebar-drawer.open {
    ...
    visibility: visible;
    transform: translateX(0);
    box-shadow: 18px 0 42px rgba(20, 35, 25, 0.18);
  }
