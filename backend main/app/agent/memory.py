...
class ConversationMemory:
    ...
    def __init__(
        self, max_messages: int = MAX_HISTORY_MESSAGES, ttl: int = SESSION_TTL
    ):
        """
        初始化对话记忆
    ...
    def save_message(self, user_id: int, session_id: str, role: str, content: str):
        ...
        if not existing:
            ...
            self.redis.set(
                f"chat:exists:{user_id}:{session_id}", "1", expire=SESSION_INDEX_TTL
            )
            self.redis.expire(index_key, SESSION_INDEX_TTL)
    ...
    def replace_history(self, user_id: int, session_id: str, messages: list[dict]):
        """使用持久化消息重建 Redis 活跃上下文。"""
        key = self._session_key(user_id, session_id)
        self.redis.delete(key)
        values = [
            json.dumps(
