...
### 2026-07-19：用户 RAG 知识共建入口与提交页

**需求背景**

- 后端新增知识文档投稿、审核和 RAG 索引流程。
- 普通用户需要在首页与 ChatPage 都能找到知识共建入口。

**完成内容**

- 新增 `KnowledgeContributionPage.vue`，使用与现有系统一致的绿色农业主题、卡片与响应式布局。
- 支持点击或拖放选择 `.md` / `.txt` 文档，可填写可选标题并提交审核。
- 显示当前用户的投稿记录、审核状态、驳回意见和分页。
- 首页顶部导航与 Workspace 卡片新增“知识共建”入口。
- ChatPage 左侧导航新增“知识共建”入口。
- 新增 `/knowledge` 鉴权路由和完整中英文界面文案。
- 简化页面介绍，移除流程步骤卡，将上传表单与“我的提交记录”并列排布。
- ChatPage 输入区的附件菜单支持提交 `.md` / `.txt` 知识资料。
- 对话投稿采用“知识 API 确定性落库 + LLM 对话说明”流程：先创建 `pending` 记录，再将真实编号和状态交给 Agent 回复。
- 当前后端 Agent 仅提供 `search_knowledge`，没有写入知识库的 Agent Tool；前端不伪装 LLM 具有未实现的写权限。
- 进一步精简知识共建页中英文标题和灰色说明文案，提交区与记录区改为上下排列。
- 排查“创建记录失败”：数据库仍在 `b1f8e8d9c3a0`，执行 `alembic upgrade head` 升级至 `bd5115dc322e`，创建 `knowledge_documents` 表。

**对接接口**

- `POST /api/knowledge/documents`：上传待审核文档。
- `GET /api/knowledge/my-submissions`：获取当前用户投稿记录。
- `GET /api/knowledge/documents`：已预留已发布知识列表 API 封装。

**待验证**

- 使用普通用户提交真实 `.md` / `.txt` 文档。
- 管理员审核后确认状态由 `pending` 变为 `approved` 或 `rejected`。
- 审核通过后在新 Agent 对话中验证 RAG 检索结果。

---

### 2026-07-19：管理中心与用户菜单组件化

**完成内容**

- 将 HomePage 内联的用户下拉拆分为 `UserMenu.vue`，保留账号信息、检测统计和退出登录。
- 只对 `admin` 角色显示“管理中心”入口。
- 新增 `/admin` 鉴权路由和前端管理员守卫，后端仍使用 `require_admin` 做最终鉴权。
- 新增 `AdminPage.vue`，仅保留“用户管理”和“知识审核”两个 Tab。
- 用户管理支持列表、新建、编辑基本信息与角色、禁用和恢复账号；禁用是后端软删除，恢复后用户可重新登录，原有历史仍保留。
- 修复合并后用户 Store 与页面命名不一致：恢复基于 `roles` 的 `isAdmin` getter 和 `fetchUserProfile()`，管理员身份、入口及路由守卫可正常识别。
- Chat 检测结果卡片新增严重程度评估入口：复用后端 `GET /api/history/severity-questions` 动态渲染问卷，并通过 `POST /api/history/tasks/{task_id}/severity-assessment` 提交原始类别、答案与补充说明；单图、批量、ZIP、视频和 Agent 检测共用组件，当前阶段不修改 HistoryPage。
- 知识管理支持状态筛选、通过、驳回、删除和重新索引。
- 知识审核表新增文档预览，调用 `GET /api/knowledge/documents/{id}` 读取完整文本，管理员可先阅读再审批。
- 新增 `frontend/src/api/admin.js`，并扩展 `frontend/src/api/knowledge.js` 的管理员接口封装。

**对接接口**

- `GET /api/user/list`、`POST /api/user/create`、`PUT /api/user/{id}`、`DELETE /api/user/{id}`。
- `GET /api/knowledge/admin/documents`。
- `PUT /api/knowledge/admin/{id}/approve`、`PUT /api/knowledge/admin/{id}/reject`。
- `DELETE /api/knowledge/admin/{id}`、`POST /api/knowledge/admin/{id}/reindex`。

---

### 2026-07-20：严重程度评估持久显示与检测历史升级

**完成内容**

- 新增通用 `SeverityAssessmentPanel.vue`，在单图、批量、ZIP、视频及 Agent 检测结果后展示现场问卷和严重程度评估。
- 修复评估结果只保存在组件临时状态的问题：结果卡片重新渲染、切换会话或打开历史详情时，会按 `task_id` 从历史详情接口恢复已保存评估。
- HistoryPage 列表以实际病害类别为标题，同时展示植物、全部类别数量、严重程度和用户处理进度。
- 筛选条件新增病害类别、严重程度和处理状态；关键词改为由后端参与数据库查询，不再只筛选当前页。
- 检测技术状态与治疗进度分离，用户可在列表或详情中将记录更新为待处理、处理中、观察中、已处理或已解决。
- History 详情页接入真实 `severity_assessments`，可查看已有评估或重新填写问卷，不再显示接口缺失占位。
- 摘要卡调整为累计任务、今日新增、高风险记录和待跟进处理，更符合病害管理场景。

**对接接口**

- `GET /api/history/severity-questions`：加载后端维护的问卷。
- `POST /api/history/tasks/{task_id}/severity-assessment`：生成并保存严重程度评估。
- `GET /api/history/tasks/{task_id}`：恢复评估、类别和处理状态。
- `GET /api/history/tasks`：使用 `keyword`、`class_name`、`risk_level`、`treatment_status` 进行服务端筛选。
- `PATCH /api/history/tasks/{task_id}/treatment-status`：更新用户维护的处理进度。

**验证结果**

- `npm run build`：通过。
- `backend/tests/test_history.py`：21 项测试通过。
- 构建仅保留第三方 `@vueuse/core` PURE 注释及大 chunk 的既有警告，不影响产物生成。

**当前边界**

- 已保存评估的结论会恢复；后端详情响应目前不返回原问卷答案，因此重新评估时不能完整回填上一次选项。
- 当前严重程度由后端可解释规则生成，前端使用 AI 风格交互但不伪造未实现的 LLM 评估调用。

---

### 2026-07-20：位置授权、天气展示与降级流程

**完成内容**

- 首页天气改为用户点击后主动触发浏览器定位授权，不再首次进入页面就静默请求外部定位。
- 精确定位成功后使用 Open-Meteo 获取当前温度和天气状态，并在本地缓存 20 分钟。
- 精确定位被拒绝、超时、浏览器不支持或页面不是安全上下文时，自动降级为 IP 估算位置；IP 与天气源均失败时显示可重试状态，不阻塞首页其他功能。
- History 详情新增“位置与环境风险”组件，可把浏览器定位保存到当前 `task_id`，并显示环境风险等级、天气摘要、指标和建议。
- 历史详情提供两种 fallback：IP 估算位置，以及手动填写地点名称、纬度和经度。
- 已有位置的记录支持重新请求天气风险；天气源返回 `unavailable` 时保留已保存位置，并提示稍后重试。
- 完成中英文界面、移动端布局和经纬度范围校验。
- 历史详情天气卡片增加风险等级局部配色、低至严重的横向风险刻度，以及湿度、温度、降雨概率和三日降水量四个带进度条的指标小卡片。

**对接接口**

- `PATCH /api/history/tasks/{task_id}/location`：保存位置，默认同步计算天气风险。
- `GET /api/history/tasks/{task_id}/weather-risk`：使用已保存位置重新分析环境风险。
- 首页当前天气仍使用前端公开天气源；后端 `/api/common/weather` 尚未实现时不影响 fallback。

**浏览器限制**

- 浏览器精确定位通常只允许 HTTPS 或 `localhost`。手机通过 `http://局域网IP` 访问时会进入 IP 估算或手动经纬度方案；正式移动端部署应配置 HTTPS。
- IP 估算只代表网络出口的大致位置，界面会明确标记为估算结果，不把它伪装成精确定位。
...
### 2026-07-20：历史记录、首页天气、权限与实时检测完善

**完成内容**

- 历史记录增加单条报告下载入口，支持 PDF 和 HTML，直接对接现有报告下载接口。
- 首页精确定位成功后改用前端 IP 定位结果补充城市名称；天气仍使用精确坐标请求，不再把经纬度显示成“当前位置”。
- 普通用户不再显示训练入口，并为 `/training` 增加管理员路由保护；训练工程文件继续保留。
- 历史详情的处理状态支持填写和再次编辑处理备注，并展示后端保存的 `treatment_note`。
- 历史筛选去除检测方式和检测场景，增加可选择或输入的植物名称筛选，使用后端现有 `plant_name` 参数。
- 摄像头 WebSocket 补充登录 Token 和显示语言参数，完善 1008 鉴权失败、异常断开及主动关闭提示；恢复 Vite WebSocket 代理。
- Vite 开发服务器监听局域网地址，便于手机访问。手机摄像头和精确定位仍需 HTTPS 或 localhost 安全上下文。

**对接接口**

- `GET /api/history/tasks/{task_id}/report/download?format=pdf|html`
- `GET /api/history/tasks?plant_name=...`
- `PATCH /api/history/tasks/{task_id}/treatment`，提交 `status` 与可选 `note`
- `WS /api/detection/camera?token=...&display_language=zh|en`

**问题与解决**

- 实时检测连接失败的主要原因是前端未发送后端要求的 Token，后端会以 WebSocket 1008 状态关闭连接。
- 本次合并后部分 Vite 入口、状态管理、请求工具和全局样式文件从当前分支消失，已从合并前的前端版本恢复，避免项目无法重新启动和构建。

**验证结果**

- `npm run build`：通过。
- `backend/tests/test_history.py`：25 项测试通过。
- 未修改后端业务代码。

---

### 2026-07-20：LLM 状态确认、PDF 中文字体与历史页可读性

**问题定位**

- 后端已正确读取 Qwen 配置，但近期日志显示 DashScope 返回 `Arrearage`，此前也出现过 `invalid_api_key`；当前 LLM 无法连接属于账号或密钥状态问题，不是前端 SSE 地址问题。
- 严重程度评估先执行确定性问卷规则，再尝试 LLM 增强；LLM 失败时保存 `rule-based` 结果，保证评估功能仍可使用。
- PDF 中文乱码不是 macOS 预览器导致，而是报告生成器此前仅查找 Windows 中文字体，macOS 上会回退到不包含中文的 Helvetica。

**完成内容**

- 严重程度结果增加评估来源展示，明确区分“规则评估”和“大模型增强（模型名）”，按钮文字改为中性的“开始严重程度评估”。
- PDF 报告增加 Windows、macOS、Linux/Docker 中文字体候选；本机已验证 STHeiti、Songti 和 Arial Unicode 可由 ReportLab 嵌入。
- 历史记录列表、筛选区、详情弹窗、天气风险和严重程度模块的主要字号整体提高。
- 删除历史详情中的接口实现说明和问卷保存说明；天气建议区域不再展示两条通用流程提示。

**验证结果**

- `npm run build`：通过。
- `backend/tests/test_history.py`：25 项测试通过。
- 使用现有检测记录生成并渲染 PDF 第一页：中文标题、表格和正文均清晰可见，无乱码。

**外部处理事项**

- DashScope 账号需要恢复可用状态或更换有效 API Key，并重启后端；代码无法绕过平台的欠费或密钥错误。

---

### 2026-07-20：Homepage 功能入口精简与中英文统一

**完成内容**

- 首页保留自然语言输入与附件入口，常用功能统一为 AI 智能诊断、检测历史、数据分析、知识共建四张卡片。
- 四张卡片采用同一配置与交互结构，整卡可点击，并增加明确的文字操作入口；检测历史不再依赖单独的小箭头。
- 移除首页导航、首页卡片和通用侧栏中的模型训练入口，训练页面文件与管理员直达路由继续保留。
- 快捷提示“我有哪些可用的检测模型？”改为“当前模型的准确度如何？”。
- 首页标题、提示词、卡片、功能标签、按钮、无障碍标签和页脚全部接入中英文词典。
- 桌面端四个功能入口紧凑并列显示，平板自动切换为两列，手机端切换为单列。
- AdminPage 与 Knowledge Contribution Page 的返回首页入口统一为 HistoryPage 的左箭头图标按钮，并移除管理员页重复的文字首页入口。

**验证结果**

- `npm run build`：通过。
- 搜索确认除保留的路由和未使用词典键外，不再存在可见 Training 入口或旧首页英文硬编码。

---

### 2026-07-20：历史详情风险卡片视觉调整

**完成内容**

- 天气风险卡恢复统一的 1px 外框，移除左侧加粗风险边框。
- 历史详情中的严重程度只在评估卡片内展示一次，移除标题区重复的风险标签。
- 严重程度评估卡根据低、中、高、严重风险使用绿、黄、橙、红局部配色；信息不足或未评估使用中性灰色。

---

### 2026-07-20：公开 WebPage 与现有业务前端融合

**完成内容**

- 保留协作成员新增的公开 `WebPage`，根路径 `/` 用作产品介绍页；登录后的业务首页调整为 `/home`。
- 保留新版 `LoginPage` 的登录/注册切换逻辑，`/register` 复用该页面，删除原独立注册页。
- 保留现有 ChatPage、HistoryPage、ChatSidebar、Agent store 和完整中英文词典，不引入会话置顶与检测摘要功能。
- 保留知识共建、管理员和训练权限守卫；系统内部返回首页、品牌链接统一指向 `/home`。
- 未合入会话置顶相关后端接口、ORM 字段和数据库迁移；仅保留与其无关的 Analytics 路由注册。
...
### 2026-07-22：ChatPage 手机端与抽屉侧栏适配

**完成内容**

- ChatSidebar 改为响应式抽屉：桌面端默认展开且可折叠，手机端默认收起并以覆盖方式拉出。
- 手机抽屉增加遮罩、关闭按钮、Esc 关闭和选择会话后自动收起，并补充中英文无障碍文案。
- ChatPage 顶栏增加统一菜单按钮；小屏幕隐藏次要模型状态，避免顶栏拥挤。
- 消息欢迎页在手机端改为单列建议卡，缩小标题、头像和间距，并优化消息气泡、批量图片与媒体卡片宽度。
- 输入栏缩小手机端间距，并适配底部安全区域。
...
### 2026-07-22：合并 frontend-cwy 与登录接口对齐

**完成内容**

- 保留协作成员的 LoginPage 中文界面、登录注册切换和忘记密码入口。
- 忘记密码流程对齐现有后端：邮箱申请验证码，随后提交邮箱、6 位验证码和新密码。
- 登录失败时展示后端真实错误；登录接口返回 401 时不再误提示“登录已过期”。
- 合入数据分析页面及配套 Analytics API、WebPage 功能文案和训练任务校验优化。
- Agent、RAG、History、Schema、数据库迁移和存储相关冲突全部保留当前分支版本，避免前端合并覆盖后端主体。
...
### 2026-07-22：数据分析页面中文化与布局升级

**需求背景**

- 原数据分析页在合并后缺少核心指标区，部分标题和模型指标仍为英文。
- 严重程度卡片使用固定零值，且标签、百分比和进度条未形成统一对齐。

**完成内容**

- 修改文件：`frontend/src/views/AnalyticsPage.vue`。
- 恢复四项核心指标，重新整理病害分布、检测趋势、高发病害、严重程度和模型性能区域。
- 页面固定文案、空状态、指标名称和时间格式统一为中文。
- 严重程度分布接入历史摘要的 `risk_counts`，展示严重、较高、中等、较低和待评估五档真实占比及记录数。
- 统一底部卡片高度、标题结构、列表间距、色条、数值列和进度条对齐，并补充平板与手机端响应式布局。
- 移除数据集样本量、训练日期和类别数等前端硬编码展示，避免静态数据误导。

**接口对齐**

- 保留现有 `/api/analytics/*` 数据接口。
- 新增读取现有 `GET /api/history/summary`，使用返回的 `risk_counts` 计算严重程度分布；未修改后端。

**验证结果**

- `npm run build`：通过。
- Vite 仅提示既有第三方 PURE 注释与打包体积警告，不影响构建产物生成。

---

### 2026-07-22：精简分析页与 Agent 检测解读

**完成内容**

- 移除数据分析页顶部核心指标展示区，页面直接进入病害分布与检测趋势。
- 调整检测 Agent 中英文提示词，禁止重复输出已由 YOLO 卡片呈现的目标总数、类别统计、推理耗时、Markdown 表格和标注图链接。
- `AgentResultCard.vue` 增加重复检测摘要过滤，历史会话中已保存的“📋 检测结果”段落也不再展示，仅保留大模型的病害解读、风险和处理建议。
- YOLO 检测卡片及检测数据保持不变。
...
- 将数据分析页和知识共建页的 Header 统一为检测历史页样式。
- 统一返回首页按钮、绿色品牌图标、页面标题、副标题、语言切换和右侧路由入口。
- 两页右侧统一提供“智能对话”和“历史记录”跳转按钮。
- 移动端隐藏副标题与品牌图标，路由按钮收缩为图标但继续保留跳转能力。
- 页面主体、接口请求和业务逻辑均未修改。
...
### 2026-07-22：补充零检测结果状态

**需求背景**

- 单图检测结果为 0 个目标时，原页面仍使用普通诊断卡片展示“0 个目标”，缺少清晰说明和下一步操作。

**完成内容**

- 新增 `frontend/src/components/HealthyResult.vue`，用于单图和 Agent 图片的零目标检测结果。
- 卡片展示“未检测到明显病害”，并提示遮挡、模糊或光照不足可能影响识别结果。
- 增加“检测结果仅针对当前图像”的边界说明，避免将零目标直接等同于植株完全健康。
- 增加“重新检测”按钮，事件经 `ChatMessageList.vue` 传递到 `ChatPage.vue`，复用现有 `handleQuickDetect('single')` 打开单图检测流程。
- 零目标结果不再展示普通诊断卡片和严重程度问卷；视频、摄像头、ZIP 与批量检测继续使用原有结果组件。
- 修正 `ChatMessageList.vue` 中重复的 `agent-analysis` 模板分支。

**接口变化**

- 未新增或修改后端接口，重新检测继续调用现有单图检测接口。
