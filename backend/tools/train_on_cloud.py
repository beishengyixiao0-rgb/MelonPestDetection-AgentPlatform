...
def main():
    ...
    # ── 训练参数 ──
    parser.add_argument(
        "--epochs", "-e", type=int, default=100, help="训练轮数（默认：100）"
    )
    parser.add_argument(
        "--batch", "-b", type=int, default=16, help="批次大小（默认：16）"
    )
    ...
    parser.add_argument(
        "--optimizer", type=str, default="SGD", help="优化器（默认：SGD）"
    )
    parser.add_argument(
    ...
    # ── 路径参数 ──
    parser.add_argument(
        "--data", "-d", type=str, default=DEFAULT_DATA_YAML, help="data.yaml 路径"
    )
    parser.add_argument(
        "--output", "-o", type=str, default=DEFAULT_OUTPUT_DIR, help="输出目录"
    )
    parser.add_argument(
        "--name", type=str, default=None, help="实验名称（默认：自动生成时间戳）"
    )

    # ── 数据增强参数 ──
    parser.add_argument(
        "--mosaic", type=float, default=1.0, help="Mosaic 增强概率（默认：1.0）"
    )
    parser.add_argument(
        "--mixup", type=float, default=0.0, help="MixUp 增强概率（默认：0.0）"
    )
    parser.add_argument(
        "--fliplr", type=float, default=0.5, help="水平翻转概率（默认：0.5）"
    )
