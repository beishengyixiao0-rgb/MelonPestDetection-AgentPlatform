...
def run_evaluation(
    ...
    # 解析评估结果
    # 使用 cast 强制将 model.names 标记为 Dict[int, str]，解决 Pylance 类型推断错误
    names_mapping: Dict[int, str] = cast(Dict[int, str], model.names)
    report = parse_evaluation_results(results, names_mapping)
...
def parse_evaluation_results(results, class_names: Dict[int, str]) -> dict:
    """
    解析 Ultralytics val() 返回结果，并进行严格的类型检查防止 Pylance 误报
    """
    # 整体指标
    report = {
        "overall": {
    ...
    np_list = results.box.np if hasattr(results.box, "np") else None

    if ap50_list:
        for i, ap50 in enumerate(ap50_list):
            class_name = class_names.get(i, f"class_{i}")
            ap50_95 = ap_list[i] if i < len(ap_list) else 0.0

            report["per_class"][class_name] = {
                "ap50": float(ap50),
                "ap50_95": float(ap50_95),
                "instances": int(np_list[i]) if np_list and i < len(np_list) else None,
            }
