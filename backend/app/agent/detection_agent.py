...
# 当前请求上下文（由 chat_stream 在每次调用前设置）
_current_user_id: int = None
_current_scene_id: int = None


def set_request_context(user_id: int = None, scene_id: int = None):
    """设置当前请求的用户和场景上下文，供工具函数使用"""
    global _current_user_id, _current_scene_id
...
- 简洁专业，不要过度解释"""
    ...
    async def chat_stream(
        ...
        Args:
            message: 用户文本消息
            image_path: 附带的图片路径（可选）
            user_id: 当前用户 ID（用于保存检测结果到数据库）
            scene_id: 检测场景 ID（用于选择模型）
